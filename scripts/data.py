...
class Col:
    ...
    ## Colunas novas
    COD_INADEMPLENTE = 'cod_inadimplente'
...
## 3.5 Colocar a coluna nova tipo um SERIAL (um número único para cada categoria/status/tipo de pagamento)

def create_serial_for_column(df, column_name, new_column_name):
    df[new_column_name] = df[column_name].astype('category').cat.codes

create_serial_for_column(df, Col.INADIMPLENTE, Col.COD_INADEMPLENTE)